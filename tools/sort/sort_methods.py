...
class SortFace(SortMethod):
    ...
    def _sort_filelist(self) -> None:
        ...
        for filename, image, alignments in self._iterator():
            ...
            faces.append(face)
            alignments_l.append(alignments)

            if len(filenames) == 16: # TODO: dynamically adjust based on VRAM
                self.score_image(filenames, faces, alignments_l)
                filenames = []
                faces = []
                alignments_l = []
    ...
    def score_image(self, filenames, faces, alignments_l) -> None:
        ...
        if len(faces):
            ...
            for embedding, filename, alignments in zip(embeddings, filenames, alignments_l):
