...
from lib.model.normalization import (
    AdaInstanceNormalization, GroupNormalization, InstanceNormalization, LayerNormalization, RMSNormalization)
...
class Decoder():  # pylint:disable=too-few-public-methods
    ...
    def __call__(self) -> tf.keras.models.Model:
        ...
        var_x = Conv2DBlock(3, kernel_size=self._config["dec_output_kernel"], 
                            name="face_out", dtype="float32")(var_x)
        
        outputs = [var_x]
        if self.learn_mask:
            var_y = Conv2DBlock(1,
                                kernel_size=self._config["dec_output_kernel"],
                                name="mask_out",
                                dtype="float32")(var_y)
